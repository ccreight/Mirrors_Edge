...
void LoadActor(const rapidjson::Value& actorValue, Game* game, Actor* parent)
{
	if (actorValue.IsObject())
	{
		...
		if (type == "Block")
		{
			...
			bool isMirror = false;
			GetBoolFromJSON(actorValue, "mirror", isMirror);
			block->SetIsMirror(isMirror);

			bool canRotate = false;
			GetBoolFromJSON(actorValue, "rotates", canRotate);
			block->SetCanRotate(canRotate);

		}
		else if (type == "Player")
		{
			// TODO: Handle construction of a player!
			Player* p = new Player(game, parent);
			actor = p;
			game->mPlayer = p;
		...
		// TODO: Add else ifs for other actor types
			...
			Checkpoint* c = new Checkpoint(game, parent);
			actor = c;
			...
			std::string str2;
			if (GetStringFromJSON(actorValue, "text", str2)) {
				c->SetText(str2);
			}
			...
			SecurityCamera* s = new SecurityCamera(game, parent);
			actor = s;

			Quaternion q;
			GetQuaternionFromJSON(actorValue, "startQ", q);
			s->SetStartQuat(q);
			GetQuaternionFromJSON(actorValue, "endQ", q);
			s->SetEndQuat(q);
			float f;
			GetFloatFromJSON(actorValue, "interpTime", f);
			s->SetInterpTime(f);
			GetFloatFromJSON(actorValue, "pauseTime", f);
			s->SetPauseTime(f);
